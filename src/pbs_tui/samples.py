...
def sample_snapshot(now: datetime | None = None) -> SchedulerSnapshot:
    ...
    jobs = [
        Job(
            ...
            state="R",
            exec_host="nid000001",
            account="climate_research",
            create_time=now - timedelta(hours=3, minutes=15),
            queue_time=now - timedelta(hours=3, minutes=10),
            eligible_time=now - timedelta(hours=3, minutes=5),
            ...
            location="Ocean cluster staging",
        ),
        Job(
            id="104830.aqua",
            name="fom_analysis",
            ...
            queue="prod",
            state="Q",
            account="analysis_lab",
            create_time=now - timedelta(minutes=45),
            queue_time=now - timedelta(minutes=45),
            eligible_time=now - timedelta(minutes=30),
            estimated_start_time=now + timedelta(minutes=15),
            walltime="01:00:00",
            nodes="1:ppn=64",
            resources_requested={"nodes": "1:ppn=64", "walltime": "01:00:00"},
            location="Analysis queue bay",
        ),
        Job(
            id="104731.vesta",
            name="vis_debug",
